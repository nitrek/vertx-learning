...
public class MainVerticle extends AbstractVerticle {
    ...
    public void start(Promise<Void> start) {
        ...
        doConfig().compose(this::storeConfig).compose(this::deployOtherVerticles).setHandler(start::handle);
    }

    /**
     * Set up and execute the {@link ConfigRetriever} to load the configuration for
     * the application
     * 
     ...
     * @return A {@link Future} of type {@link Void} indication the success or
     ...
     * @return A {@link Future} which is resolved once both of the Verticles are
     *         deployed
     */
    Future<Void> deployOtherVerticles(Void unused) {
        DeploymentOptions opts = new DeploymentOptions().setConfig(loadedConfig);

        Future<String> dbVerticle = Future
                .future(promise -> vertx.deployVerticle(new DatabaseVerticle(), opts, promise));
